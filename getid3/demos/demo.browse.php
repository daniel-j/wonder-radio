	...
	if ($handle = opendir($listdirectory)) {
		...
		foreach ($DirectoryContents as $dirname => $val) {
			...
			if (isset($_REQUEST['ShowMD5'])) {
				...
				echo '<th colspan="3">MD5&nbsp;Data (<a href="'.htmlentities($_SERVER['PHP_SELF'].'?listdirectory='.rawurlencode(isset($_REQUEST['listdirectory']) ? $_REQUEST['listdirectory'] : '.').'&ShowMD5=1', ENT_QUOTES | ENT_SUBSTITUTE, 'ISO-8859-1').'">enable</a>)</th>';
			}
			echo '<th>Tags</th>';
